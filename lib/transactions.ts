...
export function get_pay_txn(suggestedParams, from, to, amount, note) {
    return {
        from: from,
        to: to,
        type: 'pay',
...
export function get_asa_optin_txn(suggestedParams, addr, id, note) {
    return get_asa_xfer_txn(suggestedParams, addr, addr, id, 0, note)
}

export function get_asa_xfer_txn(suggestedParams, from, to, id, amt, note) {
    return {
        from: from,
        to: to,
        assetIndex: id,
...
export function get_asa_optout_txn(suggestedParams, addr, id, creator, note) {
    return get_asa_xfer_txn_out(suggestedParams, addr, addr, id, creator, 0, note)
}

export function get_asa_xfer_txn_out(suggestedParams, from, to, id, creator, amt, note) {
    return {
        from: from,
        to: to,
        assetIndex: id,
